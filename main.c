...
void handleEvents(gameState *state){
    ...
    if(SDL_PollEvent(&event)){
        ...
        default:break;
            ...
            if(((event.button.x > 119 ) && (event.button.x <212))&& ((event.button.y > 84)&&(event.button.y <118))) {
        printf("le jeu se lance");
        }
        else if(((event.button.x > 117 ) && (event.button.x <217))&& ((event.button.y > 140)&&(event.button.y <177))) {
        SDL_Quit();
        }
    }
...
void cutBitmapTexture(game *myGame,gameState state){
        ...
        myGame->g_surface = IMG_Load("./Assets/fondjeu.png");//Chargement de l'image png
